...
def gspclustering_event2(event,delta_p,sigma):
  ...
  winL = 200 # this define  number of observations in a window, the algorthm works in a sliding window manner-changed from 1000
...
def pair_clusters_appliance_wise(Finalcluster, data_vec, delta_p, instancelimit):
    ...
    DelP = [round(data_vec[i+1][0] - data_vec[i][0],2) for i in range(0,len(data_vec)-1)]
    Newcluster_1 = []
    Newtable = []
    #instancelimit = 20
